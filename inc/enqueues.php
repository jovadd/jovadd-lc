...
function jovadd_lc_get_css_url (){
    //onboarding: if no CSS custom bundle was created, serve the default one
    if (get_theme_mod("css_bundle_version_number", 0) == 0) return get_stylesheet_directory_uri() . '/'. jovadd_lc_get_css_optional_subfolder_name() . jovadd_lc_get_base_css_filename(); 

    //standard case
    return get_stylesheet_directory_uri() . '/' . jovadd_lc_get_css_optional_subfolder_name() . jovadd_lc_get_complete_css_filename(); 

}

if (!function_exists('jovadd_lc_get_css_optional_subfolder_name')):
    function jovadd_lc_get_css_optional_subfolder_name() { return "css-output/"; }
endif;

if (!function_exists('jovadd_lc_get_base_css_filename')):
    function jovadd_lc_get_base_css_filename() { return "bundle.css"; }
endif;

if (!function_exists('jovadd_lc_get_complete_css_filename')):
    function jovadd_lc_get_complete_css_filename() { 
        $filename = jovadd_lc_get_base_css_filename();
...
function jovadd_lc_get_css_version(){ 
...
add_action( 'wp_enqueue_scripts',  function  () {
    ...
    wp_enqueue_style( 'jovadd-lc-styles', jovadd_lc_get_css_url() . '#handlecsserror', array(), jovadd_lc_get_css_version()); 
...
add_action( 'wp_head', 'jovadd_lc_add_header_code' );
function jovadd_lc_add_header_code() {
    if (!get_theme_mod("jovadd_lc_fonts_header_code_disable")) {
        echo  get_theme_mod("jovadd_lc_fonts_header_code")." ";
    }
    echo get_theme_mod("jovadd_lc_header_code");
}

//ADD THE CUSTOM FOOTER CODE (SET IN CUSTOMIZER)
add_action( 'wp_footer', 'jovadd_lc_add_footer_code' );
function jovadd_lc_add_footer_code() {
	  //if (!current_user_can('administrator'))
      echo get_theme_mod("jovadd_lc_footer_code");
}

//ADD THE CUSTOM CHROME COLOR TAG (SET IN CUSTOMIZER)
add_action( 'wp_head', 'jovadd_lc_add_header_chrome_color' );
function jovadd_lc_add_header_chrome_color() {
	 if (get_theme_mod('jovadd_lc_header_chrome_color')!=""):
        ?><meta name="theme-color" content="<?php echo get_theme_mod('jovadd_lc_header_chrome_color'); ?>" />
	<?php endif;
}

//CSS error handling ENQUEUE: if CSS bundle file is not found, trigger recompile
function jovadd_lc_add_css_error_handling($url){
...
add_filter( 'clean_url', 'jovadd_lc_add_css_error_handling', 11, 1 );

//UNRENDER-BLOCK CSS 
// as per https://www.phpied.com/faster-wordpress-rendering-with-3-lines-of-configuration/

function jovadd_lc_get_headers(){
    //add link to preload CSS bundle
    $headers = "link: <".jovadd_lc_get_css_url()."?ver=".jovadd_lc_get_css_version().">; rel=preload; as=style";
...
if(!function_exists('jovadd_lc_hints')):
    function jovadd_lc_hints() {  
        header(jovadd_lc_get_headers());
    }
endif;

if  (!get_theme_mod("disable_bootstrap")) {
    add_action('send_headers', 'jovadd_lc_hints'); 
}
...
add_action ("template_redirect",function(){
    ...
    echo "<br><br>jovadd_lc_get_headers:<br><br>". str_replace(",","<br>",htmlentities(jovadd_lc_get_headers()));
